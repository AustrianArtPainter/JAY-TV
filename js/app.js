...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `JAYFLIX - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - JAYFLIX`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
        ...
        const safeResults = allResults.map(item => {
            ...
            const hasCover = item.vod_pic && /^https?:\/\//i.test(item.vod_pic);
            const posterUrl = getPosterImageUrl(item.vod_pic);
                ...
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                    ...
                    <div class="flex h-full">
                        ...
                        <div class="relative flex-shrink-0 search-card-img-container">
                            <img src="${posterUrl}" alt="${safeName}"
                                 class="h-full w-full object-cover transition-transform hover:scale-110" 
                                 onerror="this.onerror=null; this.src='${POSTER_FALLBACK_URL}'; this.classList.add('object-contain');"
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (!['JAYFLIX-Settings', 'JAY-TV-Settings', 'LibreTV-Settings'].includes(config.name)) throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'JAYFLIX-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'JAYFLIX-Settings_' + times + '.json');
}
